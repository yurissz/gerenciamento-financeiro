...
const schema = yup.object({
    ...
    email: yup.string().required('A senha é obrigatória').email("O campo deve ser um email"),
...
export const ModalEditClient = ({ isOpen, setIsOpen, clientId }: ModalProps) => {
    ...
    const handleForm = async (inputValue: IClientsForm) => {
        try {
            ...
            const { data } = await api.put(`/updateClient/${clientId}`, {
            ...
            <section className={styles.styleModal}>
                ...
                <div>
                    <form onSubmit={handleSubmit(handleForm)}>
                        <ModalInput error={errors.nome?.message} {...register("nome")} placeholder={"Digite o nome"} style={{ paddingTop: "15px" }}>Nome*</ModalInput>
                        <p className={styles.errorText}>{errors.nome?.message}</p>
                        <ModalInput error={errors.email?.message} {...register("email")} placeholder={"Digite o email"} style={{ paddingTop: "20px" }}>Email*</ModalInput>
                        <p className={styles.errorText}>{errors.email?.message}</p>
                        <div className={styles.alingSpaces}>
                            <div className={styles.alingErrors}>
                                <ModalInput error={errors.cpf?.message} {...register("cpf")} placeholder={"Digite o CPF"} styleInput={{ width: "14em" }}>CPF*</ModalInput>
                                <p className={styles.errorText}>{errors.cpf?.message}</p>
                            </div>
                            <div className={styles.alingErrors}>
                                <ModalInput error={errors.telefone?.message} {...register("telefone")} placeholder={"Digite o telefone"} styleInput={{ width: "14em" }}> Telefone*</ModalInput>
                                <p className={styles.errorText}>{errors.telefone?.message}</p>
                            </div>
                        </div>

                        <ModalInput error={errors.endereco?.message} {...register("endereco")} placeholder={"Digite o endereço"} style={{ paddingTop: "15px" }} >Endereço</ModalInput>
                        <p className={styles.errorText}>{errors.endereco?.message}</p>
                        <ModalInput error={errors.complemento?.message} {...register("complemento")} placeholder={"Digite o complemento"} style={{ paddingTop: "15px" }} >Complemento</ModalInput>
